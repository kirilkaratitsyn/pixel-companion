...
**v0.3 — рабочий исходный код прототипа и сборки для испытаний.** Windows получает медиасессии через GSMTC, а интерфейс работает с настоящим WebSocket-сервером. На физическом Pixel сборка ещё не проверена.
...
- Обложка локально приводится к 1024 px качественным bicubic-фильтром, получает лёгкое повышение резкости и сохраняется с JPEG-качеством 92 вместо прежних 384 px со стандартным сжатием.
...
3. Установите `PixelCompanion-0.3.0.apk` на Pixel. Android может попросить разрешить установку из выбранного источника. Это APK прототипа, не приложение из Play Store.
...
- Если плеер публикует маленькую обложку, локальное увеличение делает её аккуратнее, но не может восстановить отсутствующие детали. Название трека и исполнитель не отправляются внешним сервисам для поиска картинки.
- GSMTC умеет переключать треки активного YouTube Music, но не предоставляет каталог или очередь. Выбор произвольной песни потребует отдельной интеграции с сервисом. Быстрый запуск ограничен встроенным безопасным списком и не выполняет произвольные команды с телефона.
