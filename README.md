...
**v0.2 — рабочий исходный код прототипа и сборки для испытаний.** Windows получает медиасессии через GSMTC, а интерфейс работает с настоящим WebSocket-сервером. На физическом Pixel сборка ещё не проверена.
...
3. Установите `PixelCompanion-0.2.0.apk` на Pixel. Android может попросить разрешить установку из выбранного источника. Это APK прототипа, не приложение из Play Store.
...
См. [VALIDATION.md](VALIDATION.md). v0.2 успешно запущена на Windows: локальный сервер отвечает, системный микшер обнаружил реальные сессии приложений, а команда установки приложению его текущей громкости прошла. Предыдущая сборка также получила настоящие название, исполнителя, обложку и позицию из браузерной медиасессии Windows; доставка команды Pause до отдельной тестовой сессии подтверждена её callback.
