...
**v0.4 — рабочий исходный код прототипа и сборки для испытаний.** Windows получает медиасессии через GSMTC, а интерфейс работает с настоящим WebSocket-сервером. На физическом Pixel сборка ещё не проверена.
...
3. Для большой обложки YouTube Music откройте `chrome://extensions`, включите режим разработчика, нажмите **«Загрузить распакованное расширение»** и выберите папку `browser-extension` из Windows-архива. Затем один раз обновите вкладку YouTube Music. Та же инструкция лежит в `browser-extension/INSTALL-RU.txt`.
