...
Trusted LAN prototype, HTTP on TCP 8765 and WS `/ws`. No TLS in v0.1. UDP discovery uses 8764. HTTP server accepts only loopback/RFC1918 IPv4 peers; a browser's Origin must match Host. Native requests without Origin are allowed.
...
Commands: `{id,name,sessionId,revision,value?,sourceId?}`. Names: play, pause, toggle, previous, next, seek (seconds), volume (0..1), source (SourceAppUserModelId, or null for automatic). Before media commands the server resolves pending media changes and rejects stale sessionId/revision. Source and endpoint volume are independent of track identity.
