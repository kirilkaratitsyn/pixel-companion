...
public record StateMessage(string Type, int Version, string Computer, bool Locked, long Timestamp,
    MediaState Media, SourceInfo[] Sources, string? SelectedSource, float? Volume, MixerApp[] Mixer, string? Error);
public record ClientCommand(string Id, string Name, string? SessionId, long Revision, double? Value, string? SourceId, string? MixerId, bool? Muted);
