...
(async()=>{
  ...
  try {
    ...
    assert.ok(snapshot.shortcuts?.some(shortcut=>shortcut.id==='discord'));
    assert.equal((await command('launch',{shortcutId:'not-allowed'})).ok,false);checks.push('fixed quick-launch allowlist');
