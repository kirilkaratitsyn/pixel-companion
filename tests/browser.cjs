...
(async()=>{
 ...
 try{
  ...
  await page.locator('#volume-open').click();await page.locator('#volume-slider').fill('61');await page.locator('#volume-slider').dispatchEvent('change');await page.locator('#close').click();await page.waitForFunction(()=>document.getElementById('volume-value').textContent==='61%');
  await page.locator('#play').click();await page.waitForFunction(()=>document.getElementById('status').textContent==='Сейчас играет');
  await page.locator('#settings').click();await page.locator('#blank-now').click();await page.waitForTimeout(700);assert.equal(await page.locator('#wake').isVisible(),true);
  ...
  console.log(JSON.stringify({passed:true,checks:['pairing error/success','play/pause','next','seek','volume','manual black screen persists','wake does not send command','unsupported controls','untrusted metadata rendered as text','PC lock','no session','offline controls','320px layout'],backend:'simulated WebSocket contract, not Windows'},null,2));
